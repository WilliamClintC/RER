...
tryCatch({
  ...
  stargazer(
    ...
    model.numbers = FALSE,
    dep.var.labels = c("Remittance (thousands USD)", "Log(Remittance millions USD)", "Remittance (millions USD)"),
...
tryCatch({
  stargazer(
    ...
    dep.var.labels = "Remittance (thousands USD)",
    covariate.labels = c("Sending Country GDP (millions USD)", "Receiving Country GDP (millions USD)"),
...
tryCatch({
  stargazer(
    ...
    column.labels = c("With Outliers", "Without Outliers"),
    dep.var.labels = "Log(Remittance millions USD)",
...
tryCatch({
  stargazer(
    ...
    column.labels = c("With Outliers", "Without Outliers"),
    dep.var.labels = "Remittance (millions USD)",
    covariate.labels = c("Log(Sending Country GDP)", "Log(Receiving Country GDP)"),
    notes = "Note: *p<0.1; **p<0.05; ***p<0.01",
    star.cutoffs = c(0.1, 0.05, 0.01),
    digits = 3,
